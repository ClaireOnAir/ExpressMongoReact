...
exports.nuevoProducto = async (req, res, next) => {
  ...
  try {
    if (req.file && req.file.filename) {
      // Asegúrate de que req.file exista antes de acceder a filename
  ...
  try {
    ...
    fs.access(imagePath, fs.constants.F_OK, (err) => {
      if (err) {
        ...
        return res.json({
          mensaje:
            "El producto ha sido eliminado, pero la imagen no se encontró.",
        });
      }
      ...
      fs.unlink(imagePath, (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ mensaje: "Error al eliminar la imagen del servidor" });
        }
